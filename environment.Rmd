...
- 等待Rstudio Server開啟，請聽講者解釋何謂docker
...
- 幾乎所有Linux服務，只要一行即可執行
- 差異化硬碟，充分節省頻寬和硬碟
- 啟動快速，數秒就可開啟新的服務
- Dockerfile讓人人可充分交流安裝過程

--- &vcenter .largecontent

## R and Docker

- [rocker](https://github.com/rocker)
...
- 如果`apt-get update`出現`W: Size of file /var/lib/apt/lists/http.debian.net_debian_dists_jessie_main_binary-amd64_Packages.gz is not what the server reported 9067087 9068269 `，請重新執行
...
docker run --rm -v /home/vagrant/www/R/src/contrib:/src rstudio-build Rscript -e "tools::write_PACKAGES('/src')"
```